...
BEGIN
    EXECUTE IMMEDIATE """CREATE OR REPLACEEXTERNAL TABLE `<DATASET_ID>.cssr_reports`
WITH CONNECTION `<PROJECT_ID>.us.cloud_ai_resources` 
OPTIONS(
  object_metadata = 'SIMPLE',
  uris = ['gs://<PROJECT_ID>-clinical-trials-docs/*.pdf']
);""";
    EXECUTE IMMEDIATE """CREATE OR REPLACETABLE `<DATASET_ID>.cssr_reports_chunked_pdf` AS (
  ...
  PROCESS_OPTIONS => (JSON '{\"layout_config\": {\"chunking_config\": {\"chunk_size\": 250, \"include_ancestor_headings\": true}}}')
    ...
    EXECUTE IMMEDIATE """CREATE OR REPLACETABLE `<DATASET_ID>.cssr_reports_parsed_pdf` AS (
SELECT
  uri,
...
FROM `<DATASET_ID>.cssr_reports_chunked_pdf`, UNNEST(JSON_EXTRACT_ARRAY(ml_process_document_result.chunkedDocument.chunks, '$')) json
    ...
    EXECUTE IMMEDIATE """CREATE OR REPLACETABLE `<DATASET_ID>.ClinicalTrialMasterData` AS (
SELECT
uri,
