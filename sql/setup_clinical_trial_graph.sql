CREATE OR REPLACE MODEL `meridian-dev-455515.clinical_trial_dev.EmbeddingsModel`
REMOTE WITH CONNECTION `meridian-dev-455515.us-central1.llm-connection` 
OPTIONS (
  ENDPOINT = 'text-embedding-005'
);

--CREATE OR REPLACE MODEL `meridian-dev-455515.clinical_trial_dev.EmbeddingsModelGE2`
--REMOTE WITH CONNECTION `meridian-dev-455515.us-central1.llm-connection` 
--OPTIONS (
--  ENDPOINT = 'gemini-embedding-2-preview'
--);

CREATE OR REPLACE MODEL `meridian-dev-455515.clinical_trial_dev.EmbeddingsModelMME`
REMOTE WITH CONNECTION `meridian-dev-455515.us-central1.llm-connection` 
OPTIONS (
  ENDPOINT = 'multimodalembedding@001'
);

CREATE OR REPLACE MODEL `meridian-dev-455515.clinical_trial_dev.EmbeddingsModelGE1`
REMOTE WITH CONNECTION `meridian-dev-455515.us-central1.llm-connection` 
OPTIONS (
  ENDPOINT = 'gemini-embedding-001'
);

CREATE OR REPLACE MODEL `meridian-dev-455515.clinical_trial_dev.LLMModel`
REMOTE WITH CONNECTION `meridian-dev-455515.us-central1.llm-connection`
OPTIONS (
...
CREATE OR REPLACE MODEL `meridian-dev-455515.clinical_trial_multiregion.LLMModelFlash`
REMOTE WITH CONNECTION `meridian-dev-455515.us.cloud_ai_resources`
OPTIONS (
  ENDPOINT = 'gemini-2.5-flash'
);

CREATE OR REPLACE PROPERTY GRAPH clinical_trial_dev.DrugGraph
NODE TABLES (
  -- Trial nodes
  clinical_trial_dev.Trials
    KEY (PostingID)
    LABEL Trial
    PROPERTIES (PostingID, Sponsor, StudyTitle, PreferredUMLSName),

  -- Drug nodes
  clinical_trial_dev.Drug
    KEY (drug_cui)
    LABEL Drug
    PROPERTIES (drug_cui, drug_name, preferred_name, semantic_type, atc_codes, rxnorm_code, trade_names),

  -- Disorder nodes
  clinical_trial_dev.Disorder
    KEY (disorder_cui)
    LABEL Disorder
...
EDGE TABLES (
  ...
  clinical_trial_dev.DrugDisorder
  ...
  clinical_trial_dev.TrialRequires
  ...
  clinical_trial_dev.DrugCompany
  ...
  clinical_trial_dev.TrialDisorder
  ...
  clinical_trial_dev.TrialStatus
  ...
  clinical_trial_dev.DisorderHierarchy
